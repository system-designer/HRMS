...
function loadData(searchName,searchValue){
    $("#maindatagrid").datagrid({
        url:path+"manage/HdlbManageAction.jsp",
        title:"活动类别列表",
        queryParams:{
        ...
        },
        {
            ...
            formatter:function(value,rowData,rowIndex){ 
                return "<a href='javascript:void(0)' onclick='showHd(\""+rowData.lbmc+"\")'>查看活动列表</a>";
            }
        }
        ]],
        pagination:true,
...
function deleteHdlb(){
    ...
    $.messager.confirm("提示","确认删除 "+rows.length+" 条数据？",function(val){
        if(val){
            ...
            $.post(path+"manage/HdlbManageAction.jsp",{
                mode:"DELETE",
                ids:ids
    ...
    $.messager.confirm("提示","确认保存？",function(val){
        if(val){
            $("#mainform").form("submit",{
                url:path+"manage/HdlbManageAction.jsp?mode="+mode,
                onSubmit: function(){
 ...
 * 显示该类别所有活动
 */
function showHd(lbmc){
    window.parent.addTab('manage/sthd/sthd.jsp?searchName=lbmc&searchValue='+lbmc,lbmc);
}
/**
 * 去掉字符串前后空格
 */
