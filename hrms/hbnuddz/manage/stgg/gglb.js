...
function loadData(searchName,searchValue){
    $("#maindatagrid").datagrid({
        url:path+"manage/GglbManageAction.jsp",
        title:"公告类别列表",
        queryParams:{
        ...
        },
        {
            ...
            formatter:function(value,rowData,rowIndex){ 
                return "<a href='javascript:void(0)' onclick='showGg(\""+rowData.lbmc+"\")'>查看公告列表</a>";
            }
        }
        ]],
        pagination:true,
...
function deleteGglb(){
    ...
    $.messager.confirm("提示","确认删除 "+rows.length+" 条数据？",function(val){
        if(val){
            ...
            $.post(path+"manage/GglbManageAction.jsp",{
                mode:"DELETE",
                ids:ids
    ...
    $.messager.confirm("提示","确认保存？",function(val){
        if(val){
            $("#mainform").form("submit",{
                url:path+"manage/GglbManageAction.jsp?mode="+mode,
                onSubmit: function(){
 ...
 * 显示该类别所有公告
 */
function showGg(lbmc){
    window.parent.addTab('manage/stgg/stgg.jsp?searchName=lbmc&searchValue='+lbmc,lbmc);
}
/**
 * 去掉字符串前后空格
 */
