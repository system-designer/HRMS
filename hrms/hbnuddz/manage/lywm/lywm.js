...
function loadData(searchName,searchValue){
    $("#maindatagrid").datagrid({
        url:path+"manage/LywmManageAction.jsp",
        title:"留言管理列表",
        queryParams:{
...
function editLy(){
    ...
    $("#add").dialog("open");
    $("#get_lyid").val(rows[0].lyid);
    $("#lynr").val(rows[0].lynr);
    $("#lyr").val(rows[0].lyr);
    $("#hfnr").val(rows[0].hfnr);
}
/**
 *删除协会留言
 */
function deleteLy(){
    ...
    $.messager.confirm("提示","确认删除 "+rows.length+" 条数据？",function(val){
        if(val){
            ...
            $.post(path+"manage/LywmManageAction.jsp",{
                mode:"DELETE",
                ids:ids
    ...
    $.messager.confirm("提示","确认保存？",function(val){
        if(val){
            $("#mainform").form("submit",{
                url:path+"manage/LywmManageAction.jsp?mode="+mode,
                onSubmit: function(){
