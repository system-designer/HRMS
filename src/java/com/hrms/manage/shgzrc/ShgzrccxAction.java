package com.hrms.manage.shgzrc;
 ...
 * @author hp
 */
@WebServlet(name = "ShgzrccxAction", urlPatterns = {"/manage/tsrcxx/shgzrc/ShgzrccxAction.jsp"})
public class ShgzrccxAction extends HttpServlet {
    ...
    private void showOne(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            int shgzrcid = 1;
            try {
                shgzrcid = Integer.parseInt(request.getParameter("shgzrcid"));
            } catch (Exception e) {
                e.printStackTrace();
                shgzrcid = 1;
            }
            ...
            String jbxxsql = "SELECT  `zzmm`.`zzmmmc`,`jbxx`.`hjszd`, `xmfl`.`xmflmc`,  `jbxx`.`zjhm`,  `jbxx`.`gzdwid`,  `jbxx`.`xm`,   `jbxx`.`xb`, `jbxx`.`zp`,  `jbxx`.`csrq`,   `jbxx`.`mz`,   `jbxx`.`jg`,   `gzdw`.`dwmc`,   `jl`.`xxjl`,   `jl`.`gzjl`,   `jl`.`jcqk`,   `xw`.`xwmc`,   `xl`.`xlmc`,   `jbxx`.`gzsj`,   `jbxx`.`rdsj`,   `jbxx`.`zw`,   `zj`.`zjmc`,   `jbxx`.`lxdh`,   `jbxx`.`txdz`,   `jbxx`.`jkzk`,   `jbxx`.`hyzkbm`,   `jbxx`.`zymc`,   `jbxx`.`byxx`,   `jbxx`.`yzbm` FROM   `jbxx`  LEFT  JOIN `shgzrc` ON `shgzrc`.`ryid` = `jbxx`.`ryid` LEFT  JOIN `zzmm` ON `jbxx`.`zzmmbm` = `zzmm`.`zzmmbm` LEFT  JOIN `gzdw` ON `jbxx`.`gzdwid` = `gzdw`.`gzdwid`   LEFT JOIN `xmfl` ON `jbxx`.`xmflbm` = `xmfl`.`xmflbm`  LEFT  JOIN `xw` ON `jbxx`.`xwbm` = `xw`.`xwbm` LEFT JOIN jl ON jl.ryid = jbxx.ryid  LEFT JOIN `xl` ON `jbxx`.`xlbm` = `xl`.`xlbm` LEFT  JOIN `zj` ON `jbxx`.`zjbm` = `zj`.`zjbm` where `jbxx`.`ryid`=?;";
            emm.setPreparedParameter(shgzrcid);
            List list = emm.executeQuery(jbxxsql);
            request.setAttribute("list", list);
            request.getRequestDispatcher("/manage/tsrcxx/shgzrccx/shgzrcxq.jsp").forward(request, response);
    ...
    private void getTables(HttpServletRequest request, HttpServletResponse response) {
        ...
        hyb.reallyClose();
        String json = ComboboxUtil.createBatchComboJSON(dwxzlist, ssxtlist, zzmmlist, xllist, dwlblist);
    ...
    private void showList(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            int nl1 = 0;
            int nl2 = 100;
            String sql = "SELECT  `jbxx`.`ryid`, `jbxx`.`xm`,   `jbxx`.`xb`,   `gzdw`.`dwmc`,   `xl`.`xlmc`,   `jbxx`.`csrq`,   `zzmm`.`zzmmmc` FROM   `shgzrc`  left JOIN `jbxx` ON `shgzrc`.`ryid` = `jbxx`.`ryid` left JOIN `xl` ON `xl`.`xlbm` = `jbxx`.`xlbm`  left JOIN `zzmm` ON `jbxx`.`zzmmbm` = `zzmm`.`zzmmbm`  left JOIN `gzdw` ON `jbxx`.`gzdwid` = `gzdw`.`gzdwid` where ";
            String where = "";
